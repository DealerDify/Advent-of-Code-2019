...
while True:
    b=str(a[i])
    if int(b[-2:]) == 1:
        p1 = get_value(b[:-2],1,a,i)
        p2 = get_value(b[:-2],2,a,i)
        if get_mode(b[:-2],3):
            a[i+3] = p1+p2
        else:
            a[a[i+3]]= p1+p2
