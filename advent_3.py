...
for cesta in c1:
    ...
    if cesta[0]=="R":
        starty+=int(cesta[1:])
        board.append([startx,starty,delka])
    if cesta[0]=="L":
        starty-=int(cesta[1:])
        board.append([startx,starty,delka])
    if cesta[0]=="U":
...
for cesta in c2:
    ...
    if cesta[0]=="R":
        starty+=int(cesta[1:])
        board2.append([startx,starty,delka])
    if cesta[0]=="L":
        starty-=int(cesta[1:])
        board2.append([startx,starty,delka])
    if cesta[0]=="U":
...
#from: https://stackoverflow.com/users/1212596/paul-draper
...
for i in range(len(board)-1):
    for j in range(len(board2)-1):
        if intersect(board[i],board[i+1],board2[j],board2[j+1]):
            x,y = line_intersection((board[i], board[i+1]), (board2[j], board2[j+1]))
            idk.append(abs(x)+abs(y))
            delka = abs(abs(x) - abs(board[i][0])) + abs(abs(y) - abs(board[i][1]))
            delka += abs(abs(x) - abs(board2[j][0])) + abs(abs(y) - abs(board2[j][1]))
            delka +=board[i][2]+board2[j][2]
            idk2.append(delka)

print min(idk)#1st part
print min(idk2)#2nd part
